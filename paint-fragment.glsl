...
float noise(in vec2 p){
  ...
  :vec2(
    0.,
  1.);// vec2 of = 0.5 + 0.5*vec2(sign(a.x-a.y), sign(a.y-a.x));
  vec2 b=a-o+K2;
  vec2 c=a-1.+2.*K2;
...
float sdGeometry,vec3 inpColor,vec4 brushColor){
  float posInLineY=
  (uvLine.y/lineSize.y);// position along the line. in the line is 0-1.
...
vec2 p2_,float lineWidth){
  ...
  // moving around
  //    uvLine.x += (noise01(uvLine * res * 0.18)-0.5) * 0.0035;// HP random
  //    noise makes it look less scientific
...
void mainImage(out vec4 fragColor,in vec2 fragCoord){
  ...
  col=
  colorBrushStrokeLine(uv,col,vec4(vec3(.8,.1,0),.9),// red fixed line

...
struct AlloApp : public DistributedAppWithState<SharedState> {
    ...
    Parameter homing{"/homing", "", 0.1, "", 0.0, 2.0};
    Parameter aligning{"/aligning", "", 0.1, "", 0.0, 2.0};
    Parameter clustering{"/clustering", "", 0.1, "", 0.0, 2.0};
    Parameter avoidance{"/avoidance", "", 0.1, "", 0.0, 2.0};
    ...
    void onCreate() override {
        ...
        gui << background << moveRate << homing << aligning << clustering << avoidance
            << localRadius << k << fieldStrength << thickness << tailLength;
        gui.init();

        // DistributedApp provides a parameter server.
        // This links the parameters between "simulator" and "renderers"
        // automatically
        parameterServer() << background << moveRate << homing << aligning << clustering << avoidance
                          << localRadius << k << fieldStrength << thickness << tailLength;

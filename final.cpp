            ...
            for (int n = 0; n < flock.size(); n++) {
                ...
                for (int i = 0; i < f.population; i++) {
                    if (f.agent[i].isActive() == true) {
                        ...
                        int results =
                            query(space, f.agent[i].pos() * space.dim(),
                ...
                for (int i = 0; i < f.population; i++) {
                    if (f.agent[i].isActive() == true) {
                        ...
                        osc.send("/pos", n, i, (float)pos.x, (float)pos.y,
                                 (float)pos.z);
                    }
                }
        ...
        for (int i = 0; i < FLOCK_COUNT * FLOCK_SIZE; i++) {
            if (state().agent[i].active == true) {
                ...
                for (int j = headVertex + TAIL_LENGTH; j >= headVertex; j--) {
                    if (mesh.texCoord2s()[j].y != 1)
                        mesh.vertices()[j].lerp(mesh.vertices()[j - 1],
                                                tailLength);

                    if (mesh.texCoord2s()[j].y == 1) {
                        mesh.vertices()[j].set(state().agent[i].position);
                    }
                }
                ...
                mesh.texCoord2s()[i * TAIL_LENGTH].set(0.3, 4);
            }
        }
    }
